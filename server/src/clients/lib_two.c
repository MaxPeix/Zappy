...
char **duplicate_args(char **args)
{
    ...
    if (new_args == NULL) {
        perror("Erreur lors de l'allocation de la mémoire pour new_args");
        printf("Failed to allocate memory for new_args. Exiting duplicate_args\n");
        return NULL;
    }
    int i = 0;
    for (i = 0; i < arg_count; i++) {
        ...
        if (new_args[i] == NULL) {
            perror("Erreur lors de la duplication des args");
            break;
        }
    }
    if (i != arg_count) {
...
void remove_executed_command(client_t *client, int command_index)
{
    ...
    command_t *new_commands = realloc(client->commands, sizeof(command_t) * client->command_count);
...
void add_command_to_client(client_t *client, command_t *new_command)
{
    if (!client || !new_command)
        return;
    command_t *new_commands = realloc(client->commands, sizeof(command_t) * (client->command_count + 1));
    if (!new_commands) {
        free(new_command);
        return;
    }
